import { activeWordIndex, CaptionCue, exportJson, exportSrt, exportVtt, findActiveCue, HighlightMode, parseCaptions } from './core/captions.js';
...
const els = {
  ...
  captionFile: byId<HTMLInputElement>('caption-file'),
  lyricsFile: byId<HTMLInputElement>('lyrics-file'),
  lyrics: byId<HTMLTextAreaElement>('lyrics'),
  lyricsCount: byId<HTMLSpanElement>('lyrics-count'),
  captionCount: byId<HTMLSpanElement>('caption-count'),
  ...
  prevLine: byId<HTMLDivElement>('prev-line'),
  activeLine: byId<HTMLDivElement>('active-line'),
  nextLine: byId<HTMLDivElement>('next-line'),
  timeline: byId<HTMLDivElement>('timeline'),
  activeCue: byId<HTMLSpanElement>('active-cue'),
  sampleBtn: byId<HTMLButtonElement>('sample-btn'),
  offset: byId<HTMLInputElement>('offset'),
  offsetMinus: byId<HTMLButtonElement>('offset-minus'),
  offsetPlus: byId<HTMLButtonElement>('offset-plus'),
  highlightMode: byId<HTMLSelectElement>('highlight-mode'),
...
function bindEvents(): void {
  ...
  els.highlightMode.addEventListener('change', () => {
    state.highlightMode = els.highlightMode.value as HighlightMode;
    renderCaptions();
  });
...
async function handleAudioFile(): Promise<void> {
...
async function handleCaptionFile(): Promise<void> {
...
async function handleLyricsFile(): Promise<void> {
...
function renderTimeline(): void {
  ...
  for (const cue of state.cues) {
    ...
    row.querySelector<HTMLSpanElement>('.timeline-text')!.textContent = cue.text;
...
function updateTimelineSelection(): void {
  const rows = els.timeline.querySelectorAll<HTMLButtonElement>('.timeline-row');
  rows.forEach((row) => {
    row.classList.toggle('is-active', Number(row.dataset.index) === state.activeIndex);
  });

  const active = els.timeline.querySelector<HTMLButtonElement>('.timeline-row.is-active');
...
function sanitizeOffset(value: string): number {
